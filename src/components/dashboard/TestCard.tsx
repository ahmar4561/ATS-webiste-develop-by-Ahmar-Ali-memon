...
export function TestCard({
  ...
  // Keep a live clock ticking inside the card. Without this, isTestDayOpen()
  // below was only ever evaluated once at mount/render time — so a student
  // who had the dashboard open before 10:00 AM would still see "Not
  // Available Yet" even after the clock passed 10:00 AM, because nothing
  // ever told React to re-run the check. Ticking `now` every second forces
  // a re-evaluation, so the card flips to "Open Now" / "Start Test" the
  // moment the real Pakistan-time clock hits the window, no manual refresh
  // needed.
  ...
  const isOpen = behavesLikeOnline && isTestDayOpen(test.date, now);
  const isCompleted =
  ...
  const windowStatus = behavesLikeOnline ? getTestWindowStatus(test.date, now) : "locked";
  ...
  const statusBadge = () => {
    if (isPhysical && !physicalUnlocked) {
      ...
      if (test.registrationClosed && physicalRegStatus === "not_registered")
        return <Badge variant="default">Registration Closed</Badge>;
    ...
    <Card hover className="overflow-hidden">
      ...
      <CardContent className="space-y-4">
        ...
        {isPhysical && !physicalUnlocked ? (
          ...
          ) : physicalRegStatus === "loading" ? null : test.registrationClosed ? (
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-sm text-slate-600">
              <Lock className="w-4 h-4 inline mr-2" />
              Registration for this test is closed.
            </div>
          ) : (
            <div className="bg-gold-50 border border-gold-100 rounded-xl p-4 text-sm text-gold-700">
              <AlertTriangle className="w-4 h-4 inline mr-2" />
              {test.registrationNote ??
                "Register & pay to secure your seat. Can't attend in person (e.g. out of city)? Once confirmed, you can take this test online here instead."}
            </div>
          )
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-sm text-emerald-700 flex items-center justify-between gap-3 flex-wrap">
            <span className="flex items-center gap-2">
              ...
              {onDownloadIdCard
                ...
                : "Your registration is confirmed. You can take this test online here."}
            </span>
            {onDownloadIdCard && (
