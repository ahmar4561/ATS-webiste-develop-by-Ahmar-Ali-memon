...
// A single Top 10 "slot" -- one rank position, holding one student, or
// several when they're tied for that position. Tied students are shown
// together in one card instead of taking up two separate positions, so
// the list always shows exactly 10 distinct ranks (1st through 10th),
// however many students that ends up being.
interface DisplaySlot {
  rank: number;
  students: DisplayEntry[];
}

export function TodaysToppers() {
  const [entries, setEntries] = useState<DisplaySlot[] | null>(null);
  ...
  useEffect(() => {
    ...
    if (closedTest.staticTop10 && closedTest.staticTop10.length > 0) {
      ...
      // Merge students tied for the same rank into one slot, then keep
      // only the first 10 distinct rank positions (not the first 10 rows).
      const slots = groupByRank(mapped)
        .slice(0, 10)
        .map((g) => ({ rank: g.rank, students: g.entries }));
      setEntries(slots);
      setOpen(true);
      return;
    }
    ...
    fetch(`/api/merit?testId=${closedTest.id}`)
      .then((r) => r.json())
      .then((data) => {
        const meritEntries = (data.merit ?? []) as {
          rank: number;
          name: string;
          city: string;
          score: number;
        ...
        const mapped: DisplayEntry[] = meritEntries.map((e) => ({
          rank: e.rank,
          name: e.name,
          city: e.city,
          score: e.score,
        ...
        // Same merge-by-rank, top-10-positions rule as above, so a tie at
        // #9 never bumps a 10th unique student out of the popup.
        const slots = groupByRank(mapped)
          .slice(0, 10)
          .map((g) => ({ rank: g.rank, students: g.entries }));
        if (slots.length > 0) {
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 px-4 py-3 space-y-1.5">
          {entries.map((slot) => (
            ...
            >
              {/* Rank (shared by every student tied for this position) */}
              <div className="shrink-0 w-10 flex justify-center pt-0.5">
                {getRankDisplay(slot.rank)}
              </div>

              {/* Tied students stacked in the same slot */}
              <div className="flex-1 min-w-0 space-y-2">
                {slot.students.map((entry, i) => (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={cn(
                          ...
                          slot.rank === 1 ? "text-amber-800" : "text-navy-900"
                        )}
                      >
                        {entry.name}
                      </p>
                    ...
                    <div className="shrink-0 text-right">
                      <p
                        className={cn(
                          "font-bold text-sm",
                          slot.rank === 1 ? "text-amber-700" : "text-emerald-600"
                        )}
                      >
                        {entry.score} marks
                      </p>
