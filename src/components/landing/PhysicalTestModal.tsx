...
import { X, Banknote, Smartphone, MapPin, Calendar, CheckCircle2, Building2, Upload, Image as ImageIcon, Monitor, Users } from "lucide-react";
...
type PaymentMethodId = "bank" | "easypaisa";
type AttendanceModeId = "online" | "physical_plus_online";
...
const ATTENDANCE_MODES: {
...
}[] = [
  {
    id: "online",
    label: "Online Only",
    description: "Attempt the test online from home",
    fee: PHYSICAL_TEST_PAYMENT.feeOnline,
    icon: Monitor,
  },
  {
    id: "physical_plus_online",
    label: "Physical + Online",
    description: "Sit the test at the venue (+ online access)",
    fee: PHYSICAL_TEST_PAYMENT.feePhysicalPlusOnline,
    icon: Users,
  },
];
...
export function PhysicalTestModal({ test, onClose, initialValues, onRegistered }: PhysicalTestModalProps) {
  ...
  const [attendanceMode, setAttendanceMode] = useState<AttendanceModeId | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else if (trimmedName.length < 3) errs.name = "Full name is too short.";
    else if (!NAME_REGEX.test(trimmedName))
      errs.name = "Name can only contain letters (no numbers).";

    const ROLL_REGEX = /^ATS-2026-\d{5}$/i;
    const trimmedRoll = form.rollNumber.trim();
    if (!trimmedRoll) {
      errs.rollNumber = "Roll Number is required. Please enroll first to get your ATS Roll Number.";
    } else if (!ROLL_REGEX.test(trimmedRoll)) {
      errs.rollNumber = "Enter your correct ATS Roll Number in the format ATS-2026-XXXXX.";
    }
    ...
    if (!trimmedCity) errs.city = "City is required.";
    else if (!CITY_REGEX.test(trimmedCity))
      errs.city = "City can only contain letters (no numbers).";
    if (!attendanceMode) errs.attendanceMode = "Please select how you want to take the test.";
            ...
            <div className="px-6 py-10 text-center">
              ...
              {form.email.trim() && (
                <p className="text-sm text-emerald-600 font-medium mb-4">
                  📧 A registration email has been sent to <strong>{form.email.trim()}</strong>.
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <p className="text-sm font-medium text-navy-800 mb-2">
                    How will you take the test? <span className="text-red-500">*</span>
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    {ATTENDANCE_MODES.map((mode) => {
                      const Icon = mode.icon;
                      const active = attendanceMode === mode.id;
                      return (
                        <button
                          key={mode.id}
                          type="button"
                          onClick={() => {
                            setAttendanceMode(mode.id);
                            setFieldErrors((prev) => {
                              const next = { ...prev };
                              delete next.attendanceMode;
                              return next;
                            });
                          }}
                          className={cn(
                            "flex flex-col items-start gap-1 rounded-xl border-2 px-3 py-3 text-left transition-colors",
                            active
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span
                              className={cn(
                                ...
                                active ? "text-emerald-700" : "text-navy-800"
                              )}
                            >
                              {mode.label}
                            </span>
                  ...
                  {fieldErrors.attendanceMode && (
                    <p className="text-sm text-red-500 mt-1.5">{fieldErrors.attendanceMode}</p>
                  )}
                </div>
                  ...
                  <p className="text-sm font-medium text-navy-800 mb-2">
                    ...
                    {attendanceMode && (
                      <span className="text-emerald-600 font-bold ml-1">
                        (Rs. {ATTENDANCE_MODES.find((m) => m.id === attendanceMode)!.fee})
                      </span>
                    )}
                  </p>
