...
export interface TestDefinition {
  ...
  /**
   * When set, the "Register & Pay Now" button links out to this URL
   * (e.g. a Google Form) instead of opening the site's own registration
   * modal. Use this when registration/payment is being collected
   * externally and the admin will manually add + confirm the student
   * afterwards from the admin panel.
   */
  externalRegistrationUrl?: string;
  /**
   * Short blurb shown in the pre-registration info box instead of the
   * generic "Register & pay to secure your seat" text — use this to
   * mention fee / TBA date-venue for tests registered externally.
   */
  registrationNote?: string;
  /**
   * Once true, students who haven't already registered see a disabled
   * "Registration Closed" button instead of "Register & Pay Now".
   * Students who already registered (pending/confirmed/rejected) are
   * unaffected — this only blocks brand-new registrations.
   */
...
export interface PhysicalRegistration {
  ...
  /** True if a receipt exists, without carrying the (large) base64 payload. Set on list queries. */
  hasReceipt?: boolean;
