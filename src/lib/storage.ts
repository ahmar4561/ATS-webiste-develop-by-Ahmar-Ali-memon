...
/**
 * Same as getPhysicalRegistrations but deliberately excludes receipt_url —
 * that column holds the full base64 receipt image (up to 5 MB each). Pulling
 * it for every row on every admin panel load/refresh was the single biggest
 * driver of Neon network-transfer (egress) usage. Use this for the list view;
 * fetch the individual receipt only when the admin actually wants to view it
 * (see getPhysicalRegistrationReceipt below).
 */
export async function getPhysicalRegistrationsMeta(
  testId?: string
): Promise<PhysicalRegistration[]> {
  await ensureSchema();
  const { rows } = testId
    ? await sql<Omit<PhysicalRegistrationRow, "receipt_url"> & { has_receipt: boolean }>`
        SELECT id, test_id, name, roll_number, whatsapp, email, city, out_of_city,
               attendance_mode, payment_method, status, created_at,
               (receipt_url IS NOT NULL) AS has_receipt
        FROM physical_registrations WHERE test_id = ${testId} ORDER BY created_at DESC;
      `
    : await sql<Omit<PhysicalRegistrationRow, "receipt_url"> & { has_receipt: boolean }>`
        SELECT id, test_id, name, roll_number, whatsapp, email, city, out_of_city,
               attendance_mode, payment_method, status, created_at,
               (receipt_url IS NOT NULL) AS has_receipt
        FROM physical_registrations ORDER BY created_at DESC;
      `;
  return rows.map((row) => ({
    ...rowToPhysicalRegistration({ ...row, receipt_url: null }),
    hasReceipt: row.has_receipt,
  }));
}

/** Full receipt image for exactly one registration — used only when the admin clicks "View". */
export async function getPhysicalRegistrationReceipt(
  id: number
): Promise<string | null> {
  await ensureSchema();
  const { rows } = await sql<{ receipt_url: string | null }>`
    SELECT receipt_url FROM physical_registrations WHERE id = ${id};
  `;
