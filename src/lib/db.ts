...
// We use our own env var name (ATS_DB_URL) instead of the default
// POSTGRES_URL because POSTGRES_URL is owned/auto-synced by Vercel's Neon
// storage integration and can't be edited directly from the dashboard.
// Setting ATS_DB_URL as a plain env var lets us point at any Postgres
// database (a different Neon project, Supabase, etc.) without fighting
// the integration.
const connectionString = process.env.ATS_DB_URL || process.env.POSTGRES_URL;

if (!connectionString) {
  throw new Error(
    "Missing database connection string. Set ATS_DB_URL (preferred) or POSTGRES_URL in your environment variables."
  );
}

const pool = createPool({ connectionString });
// Extracting .sql without binding loses its `this` (the pool instance),
// which is what caused "Cannot read properties of undefined (reading
// 'connectionString')" — the method needs `this.connectionString` internally.
