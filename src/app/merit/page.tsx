...
export default function MeritPage() {
  ...
  // Certificates only become available once a test's 10 PM submission
  // window has actually closed for that day -- not the moment results
  // are calculated. "expired" covers today-after-10PM (in Pakistan Time);
  // a date string that's already in the past covers any earlier test day.
  // Comparing the date strings directly (rather than building Date objects
  // from the viewer's local timezone) keeps this consistent with how
  // getTestWindowStatus already reasons about "today" in Pakistan Time.
  const isCertEligible = (testId: string): boolean => {
    ...
    if (status === "locked") {
      ...
      }).format(new Date()); // en-CA gives YYYY-MM-DD, directly comparable
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <CardContent className="overflow-x-auto">
          {loading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {/* Students tied for the same score share the same rank
                    number. Every row still renders its own rank, score,
                    percentage and time cells (even when identical to the
                    row above) so no tied student's row is ever missing a
                    value -- grouping is only used for the subtle shared
                    tint below, never to hide/merge cells. */}
                {groupByRank(merit).map((group) =>
                  group.entries.map((entry, idxInGroup) => {
                    const safeScore = Number.isFinite(entry.score)
                      ? entry.score
                      : 0;
                    const safePercentage = Number.isFinite(entry.percentage)
                      ? entry.percentage
                      : 0;
                    return (
                      <tr
                        key={`${entry.testId}-${entry.rollNumber || "norollnum"}-${entry.rank}-${idxInGroup}`}
                        className={cn(
                      ...
                      >
                        ...
                        <td className="py-3 pr-4 font-bold text-emerald-600 align-top">
                          {safeScore}
                        </td>
      ...
      {certEntry && (
        <CertificateModal
          ...
          testDate={TESTS.find((t) => t.id === certEntry.testId)?.date ?? ""}
