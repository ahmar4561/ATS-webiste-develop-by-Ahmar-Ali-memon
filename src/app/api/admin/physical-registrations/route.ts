...
export async function GET(request: NextRequest) {
  ...
  const registrations = await getPhysicalRegistrationsMeta(testId);
  return NextResponse.json({ registrations });
}

/**
 * Lets the admin manually add a registration — used for tests where
 * registration/payment happens on an external Google Form instead of the
 * site's own form (e.g. the Nawabshah / Dadu online mega tests). Created
 * as "pending" by default; the admin then confirms it the same way as any
 * other registration, which unlocks that student's test on their dashboard.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { testId, name, whatsapp, rollNumber, email, city, status } = body;

    if (!testId || !name || !whatsapp) {
      return NextResponse.json(
        { error: "testId, name and whatsapp are required" },
        { status: 400 }
      );
    }

    const created = await createPhysicalRegistration({
      testId,
      name: String(name).trim(),
      rollNumber: rollNumber ? String(rollNumber).trim() : null,
      whatsapp: String(whatsapp).replace(/[^0-9]/g, ""),
      email: email ? String(email).trim() : null,
      city: city ? String(city).trim() : "N/A",
      outOfCity: false,
      attendanceMode: "online_only",
      paymentMethod: "manual",
      receiptUrl: null,
    });

    // Optionally confirm immediately if the admin already verified payment.
    if (status === "confirmed") {
      await updatePhysicalRegistrationStatus(created.id, "confirmed");
    }
