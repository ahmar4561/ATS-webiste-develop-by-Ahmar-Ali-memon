...
export default function AdminPage() {
  ...
  // Manually add a registration — for tests where sign-up happens on an
  // external Google Form (e.g. the Nawabshah / Dadu online mega tests)
  // instead of the site's own registration modal.
  const externalRegTests = TESTS.filter((t) => t.mode === "physical" && t.externalRegistrationUrl);
  const [showAddReg, setShowAddReg] = useState(false);
  const [savingAddReg, setSavingAddReg] = useState(false);
  ...
  const handleAddRegistration = async () => {
    if (!addRegForm.testId || !addRegForm.name.trim() || !addRegForm.whatsapp.trim()) {
      alert("Test, name and WhatsApp number are required.");
      return;
    }
    setSavingAddReg(true);
    try {
      const res = await fetch("/api/admin/physical-registrations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(addRegForm),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        alert(data.error ?? "Could not add registration.");
        return;
      }
      setAddRegForm({ testId: "", name: "", whatsapp: "", rollNumber: "", city: "", status: "confirmed" });
      setShowAddReg(false);
      loadRegistrations();
    } finally {
      setSavingAddReg(false);
    }
  };

  // Bulk-add — for adding a whole day's worth of paid registrations at
  // once (e.g. copy-pasted straight out of an Excel sheet) instead of
  // filling the single-entry form one student at a time. Each line is one
  // student; columns can be separated by Tab (what Excel gives you when
  // you copy cells) or by comma. Expected column order: Name, WhatsApp,
  // Roll Number (optional), City (optional). Reuses the same
  // POST /api/admin/physical-registrations endpoint as the single-add
  // form, just called once per line.
  const [showBulkAdd, setShowBulkAdd] = useState(false);
  const [bulkTestId, setBulkTestId] = useState("");
  const [bulkText, setBulkText] = useState("");
  const [bulkStatus, setBulkStatus] = useState<"pending" | "confirmed">("confirmed");
  ...
  const handleBulkAdd = async () => {
    ...
    if (lines.length === 0) {
      alert("Kam se kam ek student ki line paste karo.");
      return;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const [name, whatsapp, rollNumber, city] = cols.map((c) => (c ?? "").trim());

      if (!name || !whatsapp) {
        errors.push(`Line ${i + 1}: naam ya WhatsApp number missing hai — skip kar diya.`);
      ...
      try {
        ...
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
          errors.push(`Line ${i + 1} (${name}): ${data.error ?? "add nahi ho saka"}`);
        }
      } catch {
        errors.push(`Line ${i + 1} (${name}): network error, dobara try karo.`);
      }
  ...
  const loadRegistrations = useCallback(async () => {
    ...
    const res = await fetch("/api/admin/physical-registrations", { cache: "no-store" });
    if (res.status === 401) {
      setIsAuthed(false);
  ...
  const viewReceipt = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/physical-registrations/${id}/receipt`);
      if (!res.ok) {
        alert("Could not load receipt.");
        return;
      }
      const data = await res.json();
      // Convert the data: URI to a blob URL — browsers block direct
      // navigation to data: URIs via window.open in some cases.
      const dataUri: string = data.receiptUrl;
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => setShowAddReg((v) => !v)}>
                  <UserPlus className="w-4 h-4" />
                  Add Manually
                </Button>
                <Button variant="outline" size="sm" onClick={() => setShowBulkAdd((v) => !v)}>
                  <UserPlus className="w-4 h-4" />
                  Bulk Add
                </Button>
          ...
          {showAddReg && (
            <div className="mx-6 mb-4 p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-3">
              <p className="text-sm font-semibold text-navy-900">
                Add a registration manually (e.g. from a Google Form response)
              </p>
              <div className="grid sm:grid-cols-2 gap-3">
                <select
                  value={addRegForm.testId}
                  onChange={(e) => setAddRegForm((f) => ({ ...f, testId: e.target.value }))}
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm sm:col-span-2"
                >
                  <option value="">Select test...</option>
                  {externalRegTests.map((t) => (
                    <option key={t.id} value={t.id}>{t.title}</option>
                  ))}
                </select>
                <input
                  placeholder="Full name"
                  value={addRegForm.name}
                  onChange={(e) => setAddRegForm((f) => ({ ...f, name: e.target.value }))}
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
                />
                <input
                  placeholder="WhatsApp number"
                  value={addRegForm.whatsapp}
                  onChange={(e) => setAddRegForm((f) => ({ ...f, whatsapp: e.target.value }))}
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
                />
                <input
                  placeholder="Roll number (optional, if already enrolled)"
                  value={addRegForm.rollNumber}
                  onChange={(e) => setAddRegForm((f) => ({ ...f, rollNumber: e.target.value }))}
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
                />
                <input
                  placeholder="City (optional)"
                  value={addRegForm.city}
                  onChange={(e) => setAddRegForm((f) => ({ ...f, city: e.target.value }))}
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
                />
                <select
                  value={addRegForm.status}
                  onChange={(e) => setAddRegForm((f) => ({ ...f, status: e.target.value as "pending" | "confirmed" }))}
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm sm:col-span-2"
                >
                  <option value="confirmed">Confirmed (unlocks test immediately)</option>
                  <option value="pending">Pending (confirm later)</option>
                </select>
              </div>
              <div className="flex gap-2">
                <Button size="sm" onClick={handleAddRegistration} disabled={savingAddReg}>
                  {savingAddReg ? "Adding..." : "Add Registration"}
                </Button>
                <Button size="sm" variant="outline" onClick={() => setShowAddReg(false)}>
                  Cancel
                </Button>
              </div>
            </div>
          ...
          {showBulkAdd && (
            <div className="mx-6 mb-4 p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-3">
              <p className="text-sm font-semibold text-navy-900">
                Bulk add (Excel se copy-paste karke) — ek student per line
              </p>
              <p className="text-xs text-slate-500">
                Excel me columns is order me rakho: <strong>Name, WhatsApp, Roll Number, City</strong>{" "}
                (Roll Number aur City optional hain). Excel me poori list select karke copy karo,
                phir neeche box me paste kar do — Excel khud Tab se columns separate karke deta hai,
                usko waise hi rehne do.
              </p>
              <select
                value={bulkTestId}
              ...
              <textarea
                ...
                placeholder={"Ahmar Ali\t03001234567\tATS-2026-1234\tKarachi\nSara Khan\t03111234567\t\tDadu"}
                rows={8}
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-mono"
              />
              <select
                value={bulkStatus}
                onChange={(e) => setBulkStatus(e.target.value as "pending" | "confirmed")}
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm w-full"
              >
                <option value="confirmed">Confirmed (sabka test seat turant unlock ho jayega)</option>
                <option value="pending">Pending (baad me ek-ek karke confirm karoge)</option>
              </select>
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={handleBulkAdd} disabled={bulkRunning}>
                  {bulkRunning ? "Adding..." : "Add All"}
                </Button>
                <Button size="sm" variant="outline" onClick={() => setShowBulkAdd(false)} disabled={bulkRunning}>
                  Cancel
                </Button>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {registrations.map((r) => {
                    ...
                    >
                      <td className="py-3 pr-4 text-xs font-medium text-slate-600 max-w-[140px]">
                        {TESTS.find((t) => t.id === r.testId)?.title ?? r.testId}
                      </td>
                      {isEditing ? (
                        <>
                          ...
                          <td className="py-3 pr-4">
                            {r.hasReceipt ? (
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors disabled:opacity-50"
                              >
                                {viewingReceiptId === r.id ? "Loading..." : "View"}
                              </button>
                            ) : (
